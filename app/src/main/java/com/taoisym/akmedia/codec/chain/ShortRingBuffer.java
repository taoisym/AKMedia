package com.taoisym.akmedia.codec.chain;// Copyright 2014 Christian d'Heureuse, Inventec Informatik AG, Zurich, Switzerland
...
public class ShortRingBuffer {
    ...
    private short[] rBuf;                         // ring buffer data
     ...
     */
    public int write(short[] buf, int pos, int len) {
        if (len < 0) {
            throw new IllegalArgumentException();
        }
        if (rBufUsed == 0) {
     ...
     */
    public int write(short[] buf) {
        return write(buf, 0, buf.length);
    }

    private void append(short[] buf, int pos, int len) {
        if (len == 0) {
            return;
        }
        if (len < 0) {
     ...
     */
    public int read(short[] buf, int pos, int len) {
        if (len < 0) {
            throw new IllegalArgumentException();
        }
    ...
    private void remove(short[] buf, int pos, int len) {
        if (len == 0) {
            return;
        }
        if (len < 0) {
