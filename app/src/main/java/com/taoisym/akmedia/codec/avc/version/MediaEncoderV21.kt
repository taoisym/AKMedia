...
class MediaEncoderV21(val next:IMediaSurfaceSink):IMediaSurfaceSink, IMediaSource<NioSegment, Unit>{
    ...
    override fun setFormat(ctx: Any, format: SegmentFormat): Any? {
        ...
        codec?.run {
            configure(format.align().format(), null,null,MediaCodec.CONFIGURE_FLAG_ENCODE)
